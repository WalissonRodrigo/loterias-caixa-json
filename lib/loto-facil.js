...
exports.downloadResultadosLotoFacil = async function (customUrl) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.lotofacil;
  return await util.downloadResultadosCaixa(customUrl);
};

exports.downloadResultadoLotoFacil = async function (customUrl) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.lotofacil;
  return await util.downloadUltimoResultadoCaixa(customUrl);
};

exports.downloadConcursoLotoFacil = async function (customUrl, concurso) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.lotofacil;
    ...
    trs.each(function (index, element) {
      ...
      if (tds && tds.length > 0) {
        ...
        if (tds.length > 3) {
          ...
          sorteio.Ganhadores_14_Numeros = util.parseToInt(
            getText(tds[base + index])
          );
          sorteio.Ganhadores_13_Numeros = util.parseToInt(
            getText(tds[base + 1 + index])
          );
          sorteio.Ganhadores_12_Numeros = util.parseToInt(
            getText(tds[base + 2 + index])
          );
          sorteio.Ganhadores_11_Numeros = util.parseToInt(
            getText(tds[base + 3 + index])
          );
          sorteio.Valor_Rateio_15_Numeros = util.parseToFloat(
            getText(tds[base + 4 + index])
          );
          sorteio.Valor_Rateio_14_Numeros = util.parseToFloat(
            getText(tds[base + 5 + index])
          );
          sorteio.Valor_Rateio_13_Numeros = util.parseToFloat(
            getText(tds[base + 6 + index])
          );
          sorteio.Valor_Rateio_12_Numeros = util.parseToFloat(
            getText(tds[base + 7 + index])
          );
          sorteio.Valor_Rateio_11_Numeros = util.parseToFloat(
            getText(tds[base + 8 + index])
          );
          sorteio.Acumulado_15_Numeros = util.parseToFloat(
            getText(tds[base + 9 + index])
          );
          sorteio.Estimativa_Premio = util.parseToFloat(
            getText(tds[base + 10 + index])
          );
          sorteio.Valor_Acumulado_Especial = util.parseToFloat(
            getText(tds[base + 11 + index])
          );
