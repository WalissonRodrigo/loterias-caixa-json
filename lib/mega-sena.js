 ...
 *
 * @param {string} customUrl url da página principal da caixa, com os resultados da mega sena
 * @returns {html} relação dos resultados devolvidos pelo download
 */
exports.downloadResultadosMegaSena = async function (customUrl) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.megasena;
  return await util.downloadResultadosCaixa(customUrl);
};

exports.downloadResultadoMegaSena = async function (customUrl) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.megasena;
  return await util.downloadUltimoResultadoCaixa(customUrl);
};

exports.downloadConcursoMegaSena = async function (customUrl, concurso) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.megasena;
  ...
  return new Promise(function (resolve, reject) {
    ...
    trs.each(function (index, element) {
      ...
      if (tds && tds.length > 0) {
        ...
        if (sorteio.concurso) resultados.push(sorteio);
      } else {
        qtdErros++;
      }
