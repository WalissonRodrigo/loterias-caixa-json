...
exports.downloadResultadosLotomania = async function (customUrl) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.lotomania;
  return await util.downloadResultadosCaixa(customUrl);
};

exports.downloadResultadoLotomania = async function (customUrl) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.lotomania;
  return await util.downloadUltimoResultadoCaixa(customUrl);
};

exports.downloadConcursoLotomania = async function (customUrl, concurso) {
  customUrl = customUrl || util.URL_LOTERIAS_CAIXA.lotomania;
    ...
    trs.each(function (index, element) {
      ...
      if (tds && tds.length > 0) {
        ...
        if (tds.length > 3) {
          ...
          let index = 25;
          while (
            isNaN(util.parseToFloat(getText(tds[index]), true)) &&
            index < 100
          ) {
            index++;
          }
          ...
          if (index < 100) {
            ...
            sorteio.ganhadores_19_Numeros = util.parseToInt(
              getText(tds[index + 1])
            );
            sorteio.ganhadores_18_Numeros = util.parseToInt(
              getText(tds[index + 2])
            );
            sorteio.ganhadores_17_Numeros = util.parseToInt(
              getText(tds[index + 3])
            );
            sorteio.ganhadores_16_Numeros = util.parseToInt(
              getText(tds[index + 4])
            );
            sorteio.ganhadores_Nenhum_Numero = util.parseToInt(
              getText(tds[index + 5])
            );
            sorteio.valor_Rateio_20_Numeros = util.parseToFloat(
              getText(tds[index + 6])
            );
            sorteio.valor_Rateio_19_Numeros = util.parseToFloat(
              getText(tds[index + 7])
            );
            sorteio.valor_Rateio_18_Numeros = util.parseToFloat(
              getText(tds[index + 8])
            );
            sorteio.valor_Rateio_17_Numeros = util.parseToFloat(
              getText(tds[index + 9])
            );
            sorteio.valor_Rateio_16_Numeros = util.parseToFloat(
              getText(tds[index + 10])
            );
            sorteio.valor_Rateio_Nenhum_Numero = util.parseToFloat(
              getText(tds[index + 11])
            );
            sorteio.acumulado_20_Numeros = util.parseToFloat(
              getText(tds[index + 12])
            );
            sorteio.estimativa_Premio = util.parseToFloat(
              getText(tds[index + 13])
            );
            sorteio.valor_Acumulado_Especial = util.parseToFloat(
              getText(tds[index + 14])
            );
          }
